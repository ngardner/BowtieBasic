...
class File {
    ...
    /**
     * Upload a file
     * Use this with regular HTTP uploads. Pass in the uploaded $file, and optional directory
     */
    function upload($file,$subFolder='') {
		...
		if($subFolder) { $subFolder .= '/'.$yyyymm; } else { $subFolder = $yyyymm; }
                ...
                if($saved) {
                    ...
                    $returndata['filesize'] = filesize($this->files_fp.$subFolder.'/'.$file['name']);
     ...
     * Create a new file
     */
    function savetofile($data,$filename,$location='') {
